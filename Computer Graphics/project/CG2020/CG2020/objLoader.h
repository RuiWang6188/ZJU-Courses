...
objLoader::objLoader(const char* filename, GLuint type, GLfloat x, GLfloat y, GLfloat z, GLfloat scale)
{
	...
	if (file == NULL) {
		printf("Impossible to open the file ! Are you in the right filename ?\n");
		getchar();
		exit(0);
	}
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			else if (type == OBJTYPE2) {
				int matches = fscanf(file, "%d %d %d\n", &vertexIndex[0], &vertexIndex[1], &vertexIndex[2]);
				if (matches != 3) {
					printf("NOT TYPE2!! File can't be read by our simple parser :-( Try exporting with other options\n");
					fclose(file);
					exit(-1);
				}
				vertexIndices.push_back(vertexIndex[0]);
...
void objLoader::draw()
{
	...
	if (type == OBJTYPE1) {
		if (out_uvs.size() != out_vertices.size()) {
			cout << "ERROR: out_uvs.size() != out_vertices.size() !!" << endl;
			exit(-1);
		}

		if (out_uvs.size() % 3 != 0) {
			cout << "ERROR: out_uvs.size() % 3 != 0 !!" << endl;
			exit(-1);
		}
		...
		//for (int i = 0; i < out_uvs.size() / 3; i += 3) {
		//	glBegin(GL_TRIANGLES);
		//	//glTexCoord2f(out_uvs[i].x, out_uvs[i].y); 
		//	glVertex3f(out_vertices[i].x, out_vertices[i].y, out_vertices[i].z);
		//	//glTexCoord2f(out_uvs[i+1].x, out_uvs[i+1].y);
		//	glVertex3f(out_vertices[i+1].x, out_vertices[i+1].y, out_vertices[i+1].z);
		//	//glTexCoord2f(out_uvs[i+2].x, out_uvs[i+2].y); 
